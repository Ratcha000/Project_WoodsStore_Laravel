...
<body>
    ...
    <div class="container mt-4">
        <h2>แก้ไขสถานะ รายการสั่งทำ</h2>
        <form action="{{ route('custom_orders.update', $customOrder->Cusor_ID) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <select name="Cus_Status" class="form-control">
                    <option value="Pending" {{ $customOrder->Cus_Status == 'Pending' ? 'selected' : '' }}>Pending</option>
                    <option value="In Progress" {{ $customOrder->Cus_Status == 'In Progress' ? 'selected' : '' }}>In Progress</option>
                    <option value="Completed" {{ $customOrder->Cus_Status == 'Completed' ? 'selected' : '' }}>Completed</option>
                    <option value="Cancelled" {{ $customOrder->Cus_Status == 'Cancelled' ? 'selected' : '' }}>Cancelled</option>
                </select>
            </div>
